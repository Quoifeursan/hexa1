...
//              Manche 50 -> 99
    ...
    { id: 'aura', name: 'Menace terrifiantes', desc: 'vous réduisez les stats offensives ennemie de 5% a chaque attaque subie', unlockRound: 0, maxRound: 99, onHit: (ctx) => { ctx.monster.energie *= 0.95; ctx.monster.force *= 0.9; ctx.monster.trans *= 0.95; }},
    ...
    { id: 'timewarp', name: 'Sablier du Paradoxe', desc: "Tous les 3 tours, vous regagnez 20% de vos PV Max", unlockRound: 50, maxRound: 99, onTurn: (ctx) => { ctx.player.stacks.timewarp = (ctx.player.stacks.timewarp || 0) + 1; if (ctx.player.stacks.timewarp % 3 === 0) { const heal = Math.floor(ctx.player.maxHP * 0.2); ctx.player.hp = clamp(ctx.player.hp + heal, 0, ctx.player.maxHP); } } },
...
{ id: 'eternalflame', name: 'Flamme Éternelle', desc: "À chaque tour, gagnez +5 Force et +5 Énergie (stack infini)", unlockRound: 50, maxRound: 99, onTurn: (ctx) => { ctx.player.force += 5; ctx.player.energie += 5; ctx.player.stacks.eternalflame = (ctx.player.stacks.eternalflame || 0) + 1; } },
...
{ 
  id: 'cleanser', 
  name: 'Orbe Purificatrice', 
  desc: "Supprime immédiatement tous vos autres objets.", 
  unlockRound: 98, 
  maxRound: 102, 
  apply: (ctx) => { 
    ctx.player.passives = []; 
